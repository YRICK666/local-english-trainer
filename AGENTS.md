...
- 长期执行分流、停止条件、可自动修复的问题和安全边界见 [docs/AGENT_WORKFLOW.md](docs/AGENT_WORKFLOW.md)。
- 已声明并属于当前阶段的中间修改必须作为工作基线继续，不得仅因工作区不干净停止。
- patch、锚点、编译、测试或命令首次失败时，先分析并换方法修复；它们不是停止条件。
- 仅在文档规定的真实数据风险、未知范围修改、未知 staged 内容、高风险未授权操作或无法安全理解代码时停止。
- 已写入本文件和 `docs/AGENT_WORKFLOW.md` 的长期规则，后续提示词无需重复粘贴；提示词只需说明本轮范围与特殊限制。