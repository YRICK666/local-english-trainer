...
def test_rejects_duplicate_source_annotation_id_on_create(client: TestClient, imported_pack: dict) -> None:
    annotation = create_annotation(client)
    first = client.post("/api/vocabulary", json=build_vocabulary_payload(word="window", source_annotation_id=annotation["annotation_id"]))

    assert first.status_code == 200

    second = client.post("/api/vocabulary", json=build_vocabulary_payload(word="glass", source_annotation_id=annotation["annotation_id"]))

    assert second.status_code == 409
    assert "already linked to vocabulary item" in second.json()["detail"]


def test_duplicate_source_annotation_id_conflict_does_not_create_extra_vocabulary_item(client: TestClient, imported_pack: dict) -> None:
    annotation = create_annotation(client)
    create_vocabulary(client, word="window", source_annotation_id=annotation["annotation_id"])

    duplicate = client.post("/api/vocabulary", json=build_vocabulary_payload(word="glass", source_annotation_id=annotation["annotation_id"]))

    assert duplicate.status_code == 409

    listing = client.get("/api/vocabulary")
...
def test_patch_rejects_source_annotation_id_used_by_another_vocabulary_item(client: TestClient, imported_pack: dict) -> None:
    ...
    assert "already linked to vocabulary item" in response.json()["detail"]


def test_patch_keeps_same_source_annotation_id_without_conflict(client: TestClient, imported_pack: dict) -> None:
    annotation = create_annotation(client)
    created = create_vocabulary(client, source_annotation_id=annotation["annotation_id"])

    response = client.patch(
        f"/api/vocabulary/{created['vocab_id']}",
        json={"source_annotation_id": annotation["annotation_id"], "meaning": "updated meaning"}
    )

    assert response.status_code == 200
    assert response.json()["source_annotation_id"] == annotation["annotation_id"]
    assert response.json()["meaning"] == "updated meaning"


def test_allows_multiple_vocabulary_items_without_source_annotation_id(client: TestClient) -> None:
    first = client.post("/api/vocabulary", json=build_vocabulary_payload(word="window", source_annotation_id=None))
    second = client.post("/api/vocabulary", json=build_vocabulary_payload(word="desk", source_annotation_id=None))
